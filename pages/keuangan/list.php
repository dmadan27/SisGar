<?php
	...
	$query = "SELECT id, tgl, (CASE WHEN ket_uang='1' THEN 'Uang Masuk' ELSE 'Uang Keluar' END) as ket_uang, besaran nominal, keterangan, kode_transaksi FROM t_keuangan ORDER BY tgl desc";
	...
	//query get saldo
	$querySaldo = "SELECT saldo FROM m_saldo";
	$hasilQuerySaldo = mysqli_query($koneksi, $querySaldo);
	$dataSaldo = mysqli_fetch_assoc($hasilQuerySaldo);
			...
			<div class="col-xs-12">
				...
				<div class="box">
					...
					<div class="box-header">
						<div class="row">
							...
							<div class="col-sm-6 col-xs-12 text-right">
								<a class="btn btn-primary btn-flat margin" href="<?= base_url."index.php?m=keuangan&p=form"; ?>">Tambah Uang Masuk/Keluar</a>
							</div>
						</div>
						<div class="row">
							<div class="col-md-12">
								<h3 class="box-title">Saldo Terakhir: <?= rupiah($dataSaldo['saldo']); ?></h3>
							</div>
						</div>
					</div>
					<!-- isi panel box -->
					<div class="box-body">
						...
						<div class="row">
							<div class="col-md-12">
								<table id="tabelKeuangan" class="table table-bordered table-hover dt-responsive" cellspacing="0" width="100%">
									<thead>
										<tr>
											<th>No</th>
											<th>Tanggal</th>
									...
									<tbody>
										<?php
											...
											while($data = mysqli_fetch_assoc($hasilQuery)){
												...
												<tr>
													<td><?= ++$i ?></td>
													<td><?= cetakTgl($data['tgl']); ?></td>
													<td>
														<?php 
															if(strtolower($data['ket_uang'])=="uang masuk") echo "<span class='label label-success'>".$data['ket_uang']."</span>";
															else echo "<span class='label label-danger'>".$data['ket_uang']."</span>";
														?>
													</td>
													<td><?= rupiah($data['nominal']); ?></td>
													<td><?php if(empty(trim($data['keterangan']))) echo "-"; else echo $data['keterangan']; ?></td>
													<td>
														<?php
															if(empty($data['kode_transaksi'])){
																?>
																	<a href="<?= base_url."index.php?m=keuangan&p=form&id_keuangan=$data[id]"; ?>" class="btn btn-success btn-xs btn-flat"><i class="fa fa-edit"></i></a>
																<?php
															}
														?>
